...
dataset = pd.read_csv('trainV4.csv')
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.5, random_state = 0) #40% de datos para prueba , 60% entrenamiento se realizó asi ya que el adataset es de gran dimension


# Ajustar el clasificador de Árbol de Decisión en el Conjunto de Entrenamiento
from sklearn.tree import DecisionTreeClassifier
classifier = DecisionTreeClassifier(max_depth=5,criterion = "entropy", random_state = 0)#max depth = 6 es la profundidad de los datos y como escalara el algoritmo de regresion
